...
const findAllFiles = (inDirectory) => {
    ...
    const dir_search = (searchDir) => {
        ...
        fs.readdirSync(searchDir).forEach((file) => {
            ...
            if (stat.isDirectory())
                dir_search(filaPath);
            else if (regexes.inputFile.test(filaPath))
                results.push(filaPath);
        });
    };
...
const addNestedPath = (path) => {
    if (typeof differentRootDir === 'string')
        return normalizePath(`${differentRootDir}/${path}`);
...
(() => {
    ...
    const loadConfig = () => {
        try {
            ...
            return `Can't load config file from ${configPath}`;
        }
        return false;
    };
    ...
    let srcDirWatchDog = null;
    const coreFunction = () => {
        //	flags
        let trimPubRoot = config['trimPublicRoot'];
        if (typeof trimPubRoot !== 'boolean')
            trimPubRoot = true;
        let buildIncluded = config['buildIncluded'];
        if (typeof buildIncluded !== 'boolean')
            buildIncluded = true;
        //	data variables
        ...
        if (typeof sourceDir === 'string' && publicRoot) {
            ...
            if (watchMode && fs.existsSync(sourceDir))
        ...
        const buildTemplateFile = (srcpath, destpath) => {
            let templateHtml = '';
            try {
                templateHtml = fs.readFileSync(srcpath, { encoding: 'utf8' }).toString();
            }
            catch (error) {
                return -1;
            }
            if (templateHtml.length < 15)
                return 0;
            ...
            const buildTemplate = (templateText) => {
                ...
                allTempVars?.forEach(tempVar => {
                    ...
                    if (dataValue.length > 0) {
                        ...
                        console.warn(colorText(`Variable ${varname} not found`, 'yellow', null));
                    //	insert text to html document
                    templateText = templateText.replace(new RegExp(tempVar), dataValue);
                });
                //	trim public folder path
                if (publicRoot && trimPubRoot)
                    templateText = templateText.replace(new RegExp(`/${normalizePath(publicRoot)}/`, 'g'), '/');
                return templateText;
            };
            try {
                fs.writeFileSync(destpath, buildTemplate(templateHtml), { encoding: 'utf8' });
            }
            catch (error) {
                return -2;
            }
            ...
            if (watchMode) {
                ...
                const watchdog = fs.watch(pathObj.from, () => {
                    ...
                    changeHandler = setTimeout(() => {
                        ...
                        if (rebuildResult > 0)
                            console.log(colorText(`Rebuilt '${pathObj.from}'`, 'green', null));
                    }, fsWatch_evHold);
                });
        ...
        if (watchDirectory) {
            srcDirWatchDog = fs.watch(watchDirectory, { recursive: true }, (eventType, filename) => {
                if (eventType === 'change')
                    return;
                if (!regexes.inputFile.test(filename))
                    return;
        ...
        fs.watch(configPath, () => {
            ...
            configChangeHandler = setTimeout(() => {
                ...
                if (!configReloadResult) {
                    ...
                    console.error(colorText(configReloadResult, 'red', null));
            }, fsWatch_evHold);
        });
    }
    else {
